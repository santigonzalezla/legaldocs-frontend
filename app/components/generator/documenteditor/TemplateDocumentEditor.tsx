...
const TemplateDocumentEditor = ({ value, onChange, groupedVariables, toolbarExtra }: TemplateDocumentEditorProps) =>
{
    ...
    // Variable management (per category)
    ...
    // All visible categories = locally defined + detected from template + from registeredVars
    const detectedCats = Object.keys(groupedVariables);
    const allCats      = [...new Set([...definedCats, ...detectedCats, ...Object.keys(registeredVars)])];

    // Merge registered (panel-defined) + detected (in template) vars for a category
    ...
    // Sync editor when value changes externally (template loaded from API)
    useEffect(() => {
    ...
    /* ── Auto-highlight all {{...}} tokens in the editor (TreeWalker scan) ── */

    const scanAndHighlight = (): boolean => {
        if (!editorRef.current) return false;

        // Collect all text nodes not already inside a [data-var] span
        ...
        for (const textNode of textNodes) {
            ...
            // Keep cursor tracking correct
            const wasCursorNode = savedAnchor === textNode;
            parent.replaceChild(frag, textNode);

            // If the cursor was inside this text node, move it after the last span
    ...
    const insertVariableSpan = (varKey: string, label: string) => {
        ...
        if (selection.rangeCount > 0) {
            ...
            range = editorRef.current.contains(cur.commonAncestorContainer)
                ? cur
                : (() => { const r = document.createRange(); r.selectNodeContents(editorRef.current!); r.collapse(false); return r; })();
        } else {
    ...
    const insertComponent = (type: string) => {
        ...
        let el: HTMLElement;
        switch (type) {
            case 'heading':   el = Object.assign(document.createElement('h2'), { textContent: 'Nuevo Título' });   break;
            case 'paragraph': el = Object.assign(document.createElement('p'),  { textContent: 'Nuevo párrafo' }); break;
            case 'list': { el = document.createElement('ul'); const li = document.createElement('li'); li.textContent = 'Elemento'; el.appendChild(li); break; }
            case 'table': { el = document.createElement('table'); el.className = docStyles.table; el.innerHTML = '<tr><td>Celda 1</td><td>Celda 2</td></tr><tr><td>Celda 3</td><td>Celda 4</td></tr>'; break; }
            case 'signature': { el = document.createElement('div'); el.className = docStyles.signatureBlock; el.innerHTML = `<div class="${docStyles.signatureLine}"></div><p>FIRMA</p><p>NOMBRE</p>`; break; }
            default: el = Object.assign(document.createElement('p'), { textContent: 'Nuevo elemento' });
        }
    ...
    const toggleCat = (cat: string) =>
        setExpanded(prev => ({ ...prev, [cat]: !(prev[cat] ?? true) }));
            ...
            <div className={docStyles.editorCard}>
                ...
                <div className={docStyles.toolbar}>
                    <div className={docStyles.toolbarGroup}>
                        <button className={docStyles.toolbarButton} onClick={() => execCommand('bold')}      title="Negrita">   <Bold      className={docStyles.toolbarIcon} /></button>
                        <button className={docStyles.toolbarButton} onClick={() => execCommand('italic')}    title="Cursiva">   <Italic    className={docStyles.toolbarIcon} /></button>
                        <button className={docStyles.toolbarButton} onClick={() => execCommand('underline')} title="Subrayado"> <Underline className={docStyles.toolbarIcon} /></button>
                    </div>
                    <div className={docStyles.toolbarGroup}>
                        <button className={docStyles.toolbarButton} onClick={() => execCommand('justifyLeft')}><AlignLeft    className={docStyles.toolbarIcon} /></button>
                        <button className={docStyles.toolbarButton} onClick={() => execCommand('justifyCenter')}><AlignCenter className={docStyles.toolbarIcon} /></button>
                        <button className={docStyles.toolbarButton} onClick={() => execCommand('justifyRight')}><AlignRight  className={docStyles.toolbarIcon} /></button>
                        <button className={docStyles.toolbarButton} onClick={() => execCommand('justifyFull')}><AlignJustify className={docStyles.toolbarIcon} /></button>
                    </div>
                    <div className={docStyles.toolbarGroup}>
                        <button className={docStyles.toolbarButton} onClick={() => execCommand('insertUnorderedList')}><List        className={docStyles.toolbarIcon} /></button>
                        <button className={docStyles.toolbarButton} onClick={() => execCommand('insertOrderedList')}><ListOrdered  className={docStyles.toolbarIcon} /></button>
                    </div>
                ...
                <div className={panelStyles.editorWrapper}>
                    ...
                    <div className={panelStyles.panel}>
                        ...
                        <div className={panelStyles.tabs}>
                            <button className={`${panelStyles.tab} ${activeTab === 'variables'  ? panelStyles.tabActive : ''}`} onClick={() => setActiveTab('variables')}>Variables</button>
                            <button className={`${panelStyles.tab} ${activeTab === 'components' ? panelStyles.tabActive : ''}`} onClick={() => setActiveTab('components')}>Componentes</button>
                        </div>
                        ...
                        <div className={panelStyles.panelContent}>
                            ...
                            {activeTab === 'variables' && (
                                <>
                                    <p className={panelStyles.hint}>
                                        Escribe <code>{'{{cat:campo:tipo}}'}</code> en el editor o define las variables aquí.
                                    </p>
                                    ...
                                    {allCats.map(cat => {
                                        ...
                                        return (
                                            <div key={cat} className={panelStyles.categoryGroup}>
                                                <button className={panelStyles.categoryHeader} onClick={() => toggleCat(cat)}>
                                                    {isOpen ? <ArrowDown className={panelStyles.categoryIcon} /> : <ArrowGo className={panelStyles.categoryIcon} />}
                                                    <span className={panelStyles.categoryTitle}>{cat.replace(/_/g, ' ')}</span>
                                                    <span className={panelStyles.categoryCount}>({vars.length})</span>
                                                </button>
                                                ...
                                                {isOpen && (
                                                    <div className={panelStyles.categoryContent}>
                                                        {vars.map(v => {
                                                            ...
                                                            return (
                                                                <button key={v.full} className={panelStyles.fieldItem} onClick={() => insertVariableSpan(v.full, v.field)} title={`Insertar {{${v.full}}}`}>
                                                                    <Icon className={panelStyles.fieldIcon} />
                                                                    <div className={panelStyles.fieldInfo}>
                                                                        <span className={panelStyles.fieldLabel}>{v.field.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase())}</span>
                                                                        <span className={panelStyles.fieldType}>{v.type}</span>
                                                                    </div>
                                                                </button>
                                                            );
                                                        })}
                                                        ...
                                                        {isAddingVar ? (
                                                            <div className={panelStyles.addVarForm}>
                                                                <input
                                                                    ...
                                                                    onKeyDown={e => { if (e.key === 'Enter') handleAddVar(cat); if (e.key === 'Escape') { setAddingVarIn(null); setNewVarField(''); } }}
                                                                />
                                                                <select className={panelStyles.addVarSelect} value={newVarType} onChange={e => setNewVarType(e.target.value)}>
                                                                    {TYPES.map(t => <option key={t} value={t}>{t}</option>)}
                                                                </select>
                                                                <div className={panelStyles.addVarActions}>
                                                                    <button className={panelStyles.addVarConfirm} onClick={() => handleAddVar(cat)} disabled={!newVarField.trim()}>Insertar</button>
                                                                    <button className={panelStyles.addVarCancel} onClick={() => { setAddingVarIn(null); setNewVarField(''); }}>Cancelar</button>
                                                                </div>
                                                            </div>
                                                        ) : (
                                                            <button
                                                                className={panelStyles.addVarBtn}
                                                                onClick={() => { setAddingVarIn(cat); setNewVarField(''); setNewVarType('texto'); }}
                                                            >
                                    ...
                                    {addingCat ? (
                                        <div className={panelStyles.addCatForm}>
                                            ...
                                            />
                                            <div className={panelStyles.addVarActions}>
                                                <button className={panelStyles.addVarConfirm} onClick={handleAddCategory} disabled={!newCatName.trim()}>Crear</button>
                                                <button className={panelStyles.addVarCancel} onClick={() => { setAddingCat(false); setNewCatName(''); }}>Cancelar</button>
                                            </div>
                                        </div>
                                    ) : (
                            ...
                            {activeTab === 'components' && (
                                <div className={panelStyles.componentsSection}>
                                    <p className={panelStyles.hint}>Haz clic para insertar en la posición del cursor.</p>
                                    {COMPONENTS.map(({ type, label, description, Icon }) => (
                                        <button key={type} className={panelStyles.componentItem} onClick={() => insertComponent(type)}>
